...
class ModelCheckpoint(Hook):
    ...
    def _save_checkpoint(self, meta, out_dir, filename, is_best, create_symlink=True):
        ...
        if create_symlink or is_best:
            dst_file = os.path.join(out_dir, f"model_best_{meta['epoch']}.pth")
    ...
    def save_checkpoint(self, runner, metrics):
        ...
        if save_top_k >= 1:
            ...
            #             stats, out_dir=self.save_model_path, is_best=is_best, filename=f"last.pth.tar")
            if runner.epoch % self.print_freq == 0 or is_best:
                self._save_checkpoint(
                        stats, out_dir=self.save_model_path, is_best=is_best, filename=f"best.pth.tar")

                if not flag:
                    tqdm.write(' * Best training metrics so far@ {best_metric} in epoch {best_epoch}'.format(
                        best_metric=stats['best_metric'], best_epoch=stats['best_epoch']))
