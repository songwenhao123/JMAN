...
# from UDL.pansharpening.run_pansharpening import ml_info
from clearml import Logger
...
class EpochBasedRunner(BaseRunner):
    ...
    def run_iter(self, data_batch, train_mode, **kwargs):
        ...
        # print(self._inner_iter)
        # print(self._iter)
        # print(self._epoch)
        
        # for key, value in self.outputs["log_vars"].items():
        #     Logger.current_logger().report_scalar(key, "idx", value, self._epoch)
    ...
    def train(self, data_loader, **kwargs):
        ...
        # for i, data_batch in tqdm(list(enumerate(self.data_loader)), desc="train", leave=False):
        for i, data_batch in enumerate(tqdm(self.data_loader, desc="train", leave=False)):
        # for i, data_batch in tqdm(enumerate(self.data_loader), desc="train", leave=False):
            self._inner_iter = i
        ...
        # Logger.current_logger().report_scalar(title='loss', series='epoch', value=self.metrics['loss'], iteration=self._epoch)
    ...
    def val(self, data_loader, **kwargs):
        ...
        time.sleep(2)  # Prevent possible deadlock during epoch transition
        tic = time.time()
        # for i, data_batch in tqdm(list(enumerate(self.data_loader)), desc="val", leave=False):
        for i, data_batch in enumerate(tqdm(self.data_loader, desc="val", leave=False)):
            self._inner_iter = i
    ...
    def run(self, data_loaders, workflow, max_epochs=None, **kwargs):
        ...
        if len(workflow) ==1: # only for eval
