...
# get_logger(None, cfg, cfg_name, log_level)
def get_logger(name=None, cfg=None, cfg_name=None, phase='train', log_level=logging.INFO, file_mode='w'):  # log_file=None,
    ...
    # output
    # root_output_dir = Path(cfg.out_dir) / cfg.arch / cfg.act_variant
    root_output_dir = Path(cfg.out_dir) / cfg.arch
    
    if cfg.eval:
        # test
        if cfg.test_mode == "downsample":
            model_save_dir = root_output_dir / cfg.dataset["val"] / "low"
        elif cfg.test_mode == "fullsample":
            model_save_dir = root_output_dir / cfg.dataset["val"] / "full" 
        else:
            raise ValueError(f"test_mode must be either downsample or fullsample while got {cfg.test_mode}")
    else:
        # train
        model_save_dir = root_output_dir / cfg.dataset["train"] 
    
    for directory in [root_output_dir, model_save_dir]:
    ...
    #     model_save_dir = final_output_dir
    # else:
    #     model_save_tmp = "model"
    #     model_save_dir = final_output_dir / model_save_tmp
# ./results/net/qb/model_task 
# ./results/net/low/test_qb
