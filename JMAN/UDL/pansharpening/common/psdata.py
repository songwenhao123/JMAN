...
class PansharpeningSession():
    ...
    def get_dataloader(self, dataset_name, distributed):
        ...
        if any(list(map(lambda x: x in dataset_name, ['wv2', 'wv3', 'gf2', 'qb']))):
    ...
    def get_eval_dataloader(self, dataset_name, distributed):
        ...
        if 'valid' in dataset_name:
            ...
            from UDL.pansharpening.evaluation.ps_evaluate import \
                MultiExmTest_h5
                
            if self.args.test_mode == "downsample":
                dataset = MultiExmTest_h5('/'.join([self.args.data_dir, 'test_data', satellite.lower(),
                                                    f"{dataset_name.replace('_hp', '')}_multiExm1.h5"]),
                                                    dataset_name, img_scale=self.args.img_range)
            elif self.args.test_mode == "fullsample":
                dataset = MultiExmTest_h5('/'.join([self.args.data_dir, 'test_data', satellite.lower(),
                                                   f"{dataset_name.replace('_hp', '')}_OrigScale_multiExm1.h5"]),
                                                    dataset_name, img_scale=self.args.img_range)
            else:
