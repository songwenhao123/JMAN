...
from UDL.Basis.dist_utils import (dist_train_v1, get_dist_info, init_dist,
                                  reduce_mean)
...
def save_results(idx, save_model_output, filename, save_fmt, output, gt):
    save_name = os.path.join(f"{save_model_output}", f"{idx}_output_mulExm.mat")
    sio.savemat(save_name, {'sr': output.cpu().detach().numpy()})
    # output = showimage8(output)
    # save_name = save_name.replace('mat', 'png')
    # plt.imsave(save_name, output, dpi=300)
    # gt = showimage8(gt)
    # save_name = save_name.replace('output', 'gt')
    # plt.imsave(save_name, gt, dpi=300)
