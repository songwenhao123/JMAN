...
from mmcv.runner import (DistSamplerSeedHook, EpochBasedRunner,
                         ...
                         build_runner, find_latest_checkpoint, get_dist_info,
                         init_dist)
from mmcv.utils.logging import create_logger, print_log
from UDL.AutoDL import ModelDispatcher, build_model, getDataSession
...
def calculate_model_complexity(model, spectual_num):
    # Define input shapes
    input_shape = (spectual_num, 48, 48)
    input_data = [torch.randn(input_shape).unsqueeze(0).cuda(), torch.randn((1, 48, 48)).unsqueeze(0).cuda()]

    # Use TorchSummary to print model summary
    summary(model, [input_shape, (1, 48, 48)])
    ...
    # operator_module_flops = flops.by_module_and_operator()

    # Print FLOPs information
    print(f'# Total FLOPs: {total_flops}\n')
    # print(f'# Operator FLOPs: {operator_flops}\n')
    # print(f'# Module FLOPs: {module_flops}\n')
    # print(f'# Operator and Module FLOPs: {operator_module_flops}\n')

    # Calculate MACs and model parameters using THOP
    macs, params = profile(model, inputs=input_data)

    # Print MACs and model parameter information
    print(f'# MACs: {macs}\n')
...
def trainer(cfg, logger,
    ...
    spectual_num = 8 if 'wv3' in cfg.train_set or 'test_wv3' in cfg.val_set else 4
    calculate_model_complexity(model.model, spectual_num)
